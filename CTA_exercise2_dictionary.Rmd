...
```{r}
tweets$date <- as.Date(tweets$created_at)
tweet_corpus <- corpus(tweets, text_field = "text", docvars = c("newspaper", "date"))
toks_news <- tokens(tweet_corpus, remove_punct = TRUE)
data("data_dictionary_LSD2015", package = "quanteda")
data_dictionary_LSD2015_pos_neg <- data_dictionary_LSD2015[1:2]
toks_news_lsd <- tokens_lookup(toks_news, dictionary = data_dictionary_LSD2015_pos_neg)
dfmat_news_lsd <- dfmat_news_lsd %>%
  as.data.frame() %>%
  mutate(sentiment = positive - negative)

```{r}
dfmat_news_lsd %>%
  ggplot(aes(x = date, y = sentiment, color = newspaper)) +
  geom_line() +
  geom_smooth(method = "loess", alpha = 0.2) +
  labs(title = "Sentiment trends by newspaper",
       x = "date",
